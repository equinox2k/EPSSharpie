...
namespace com.softhub.ps.graphics
{
	...
	[Serializable]
	internal class ImageCommand : Command
	{
		...
		internal ImageCommand(Bitmap bitmap, AffineTransform xform)
		{
			this.bitmap = bitmap;
			this.xform = xform;
		}
		...
		internal override Rectangle2D Bounds2D
		{
			get
			{
				if (bounds == null)
				{
					double x = xform.TranslateX;
					double y = xform.TranslateY;
					int w = bitmap.Width;
					int h = bitmap.Height;
					Point2D v = new Point2D.Double(w, h);
					xform.deltaTransform(v, v);
					bounds = new Rectangle2D.Double(x, y, v.X, v.Y);
				}
				return bounds;
			}
		}
